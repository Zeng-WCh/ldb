...
static handler *lengine_create_handler(handlerton *hton, TABLE_SHARE *table,
                                       bool partitioned, MEM_ROOT *mem_root);

handlerton *lengine_hton;

/* Interface to mysqld, to check system tables supported by SE */
static bool lengine_is_supported_system_table(const char *db,
                                              const char *table_name,
                                              bool is_sql_layer_system_table);

#ifdef HAVE_PSI_INTERFACE
static PSI_mutex_key key_mutex_LDB_share_mutex;

static PSI_mutex_info all_ldb_mutexes[] = {
    {&key_mutex_LDB_share_mutex, "LDB_share::mutex", 0, 0, PSI_DOCUMENT_ME}};

static void init_ldb_psi_keys() {
  const char *category = "lengine";
  int count;

  count = static_cast<int>(array_elements(all_ldb_mutexes));
  mysql_mutex_register(category, all_ldb_mutexes, count);
}
#endif
LDB_share::LDB_share() {
  thr_lock_init(&lock);
  mysql_mutex_init(key_mutex_LDB_share_mutex, &mutex, MY_MUTEX_INIT_FAST);

  io = new IO();
}

static int lengine_init_func(void *p) {
  DBUG_TRACE;
  ...
  lengine_hton = (handlerton *)p;
  lengine_hton->state = SHOW_OPTION_YES;
  lengine_hton->create = lengine_create_handler;
  lengine_hton->flags = HTON_CAN_RECREATE;
  lengine_hton->is_supported_system_table = lengine_is_supported_system_table;
...
LDB_share *ha_lengine::get_share() {
  LDB_share *tmp_share;

  DBUG_TRACE;

  lock_shared_ha_data();
  if (!(tmp_share = static_cast<LDB_share *>(get_ha_share_ptr()))) {
    tmp_share = new LDB_share;
...
static handler *lengine_create_handler(handlerton *hton, TABLE_SHARE *table,
  ...
  return new (mem_root) ha_lengine(hton, table);
}

ha_lengine::ha_lengine(handlerton *hton, TABLE_SHARE *table_arg)
...
*/
static st_handler_tablename ha_lengine_system_tables[] = {
...
static bool lengine_is_supported_system_table(const char *db,
  ...
  systab = ha_lengine_system_tables;
...
int ha_lengine::open(const char *name, int, uint, const dd::Table *) {
  DBUG_TRACE;
  char name_buf[1024];
  if (!(share = get_share())) return 1;

  fn_format(name_buf, name, "", SDE_EXT, MY_REPLACE_EXT | MY_UNPACK_FILENAME);
  share->io->openTable(name_buf);
...
int ha_lengine::write_row(uchar *buf) {
  DBUG_TRACE;
  /*
  ...
  */
  ...
  share->io->writeRow(buf, table->s->rec_buff_length);
  stats.records++;
...
*/
int ha_lengine::update_row(const uchar *ori, uchar *to) {
  DBUG_TRACE;
  if (!(share = get_share())) return 1;

  share->io->updateRow(ori, to, table->s->rec_buff_length);
...
int ha_lengine::delete_row(const uchar *buf) {
  DBUG_TRACE;
  if (!(share = get_share())) return 1;
  int rc = share->io->deleteRow(buf, table->s->rec_buff_length);
  if (rc == -1) {
    return -1;
  }
  stats.records--;
...
int ha_lengine::index_read_map(uchar *, const uchar *, key_part_map,
...
int ha_lengine::index_next(uchar *) {
  int rc;
  DBUG_TRACE;
  rc = HA_ERR_WRONG_COMMAND;
  return rc;
...
int ha_lengine::index_prev(uchar *) {
  int rc;
  DBUG_TRACE;
  rc = HA_ERR_WRONG_COMMAND;
  return rc;
...
*/
int ha_lengine::index_first(uchar *) {
  int rc;
  DBUG_TRACE;
  rc = HA_ERR_WRONG_COMMAND;
  return rc;
...
/**
  ...
  opt_range.cc, opt_sum.cc, sql_handler.cc and sql_select.cc
*/
int ha_lengine::index_last(uchar *) {
  int rc;
  DBUG_TRACE;
  rc = HA_ERR_WRONG_COMMAND;
  return rc;
...
/**
  ...
  sql_update.cc
*/
int ha_lengine::rnd_init(bool) {
  DBUG_TRACE;
  offset = sizeof(ull);
  ref_length = sizeof(ull);
  stats.records = 0;
  return 0;
}

int ha_lengine::rnd_end() {
  DBUG_TRACE;
  return 0;
}
...
/**
  ...
  sql_update.cc
*/
int ha_lengine::rnd_next(uchar *buf) {
  int rc;
  DBUG_TRACE;
  if (!(share = get_share())) {
    return HA_ERR_INTERNAL_ERROR;
  }
  rc = share->io->readRow(buf, table->s->rec_buff_length, offset);
  if (rc == -1) {
    return HA_ERR_END_OF_FILE;
  }
  offset = share->io->curPos();
  return 0;
}
...
*/
void ha_lengine::position(const uchar *) {
  DBUG_TRACE;
  my_store_ptr(ref, ref_length, offset);
  return;
}
...
*/
int ha_lengine::rnd_pos(uchar *buf, uchar *pos) {
  DBUG_TRACE;
  offset = my_get_ptr(pos, ref_length);
  int rc = share->io->readRow(buf, table->s->rec_buff_length, offset);
  stats.records++;
  return rc;
}
...
/**
  ...
  ha_innodb.cc
*/
int ha_lengine::extra(enum ha_extra_function) {
  DBUG_TRACE;
  return 0;
}
...
*/
int ha_lengine::delete_all_rows() {
  DBUG_TRACE;
  if ((share = get_share()) == 0) return 1;
  share->io->deleteAllRows();
  offset = sizeof(ull);
  return 0;
}
...
*/
int ha_lengine::external_lock(THD *, int) {
  DBUG_TRACE;
  return 0;
}
...
*/
THR_LOCK_DATA **ha_lengine::store_lock(THD *, THR_LOCK_DATA **to,
...
*/
int ha_lengine::delete_table(const char *name, const dd::Table *) {
  DBUG_TRACE;
  char name_buf[1024];
  fn_format(name_buf, name, "", SDE_EXT, MY_REPLACE_EXT | MY_UNPACK_FILENAME);
  my_delete(name_buf, MYF(0));
  // share->io->closeTable();
  offset = 0;
...
*/
int ha_lengine::rename_table(const char * _from, const char * _to, const dd::Table *,
                             dd::Table *) {
  DBUG_TRACE;
  ...
  fn_format(from, _from, "", SDE_EXT, MY_REPLACE_EXT | MY_UNPACK_FILENAME);
  fn_format(to, _to, "", SDE_EXT, MY_REPLACE_EXT | MY_UNPACK_FILENAME);

  File orifile = my_open(from, O_RDONLY, MYF(0));
  File newfile = my_open(to, O_RDWR | O_CREAT | O_TRUNC, MYF(0));

  ull total = 0, read = 0, record_len = 0;
  uchar flag = 2;
  ull buf_len = 32;
  uchar buf[32];

  my_read(orifile, (uchar *)&total, sizeof(ull), MYF(0));
  my_write(newfile, (uchar *)&total, sizeof(ull), MYF(0));

  while (read < total) {
    my_read(orifile, &flag, sizeof(uchar), MYF(0));
    if (flag == 1) {
      my_read(orifile, (uchar *)&record_len, sizeof(ull), MYF(0));
      my_seek(orifile, record_len, MY_SEEK_CUR, MYF(0));
      continue;
    }
    my_write(newfile, &flag, sizeof(uchar), MYF(0));
    my_read(orifile, (uchar *)&record_len, sizeof(ull), MYF(0));
    my_write(newfile, (uchar *)&record_len, sizeof(ull), MYF(0));
    if (record_len > buf_len) {
      ull read_now = 0;
      while (record_len > 0) {
        read_now = record_len > buf_len ? buf_len : record_len;
        my_read(orifile, buf, read_now, MYF(0));
        my_write(newfile, buf, read_now, MYF(0));
        record_len -= read_now;
      }
    }
    else {
      my_read(orifile, buf, record_len, MYF(0));
      my_write(newfile, buf, record_len, MYF(0));
    }
    ++read;
  }
  my_close(orifile, MYF(0));
  my_close(newfile, MYF(0));

  my_delete(from, MYF(0));
...
*/
ha_rows ha_lengine::records_in_range(uint, key_range *, key_range *) {
  DBUG_TRACE;
...
int ha_lengine::create(const char *name, TABLE *, HA_CREATE_INFO *,
                       dd::Table *) {
  DBUG_TRACE;
  /*
  ...
  */
  ...
  fn_format(namebuf, name, "", SDE_EXT, MY_REPLACE_EXT | MY_UNPACK_FILENAME);
  if (share->io->createTable(namebuf)) return 1;
  share->io->closeTable();

  return 0;
}

struct st_mysql_storage_engine lengine_storage_engine = {
...
static SYS_VAR *lengine_system_variables[] = {
...
mysql_declare_plugin(LEngine){
    MYSQL_STORAGE_ENGINE_PLUGIN,
    &lengine_storage_engine,
    "LEngine",
    PLUGIN_AUTHOR_ORACLE,
    "L Engine",
    PLUGIN_LICENSE_GPL,
    lengine_init_func, /* Plugin Init */
    nullptr,           /* Plugin check uninstall */
    nullptr,           /* Plugin Deinit */
    0x0001 /* 0.1 */,
    func_status,              /* status variables */
    lengine_system_variables, /* system variables */
